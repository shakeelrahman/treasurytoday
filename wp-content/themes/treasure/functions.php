...
function wpb_postsbycategory($catID)
{
	...
	if ($the_query->have_posts()) {
		...
		while ($the_query->have_posts()) {
			...
			$string .= '</a>';
			$string .= '<div class="">';
			$string .= get_link_to_topics_page(get_the_ID());
			$string .= '<span class="text-muted small"> - ' . meks_time_ago(). '</span>';
...
function my_ajax_pagination()
{
	...
	$returndata .= the_posts_pagination(array(
		...
		$returndata .= '<input type="hidden" value="'.get_query_var('paged').'" id="wacurrentpageno">' ;
        $returndata .= '</div>';
 ...
 */
$first_image = $featured_img_url = get_the_post_thumbnail_url($post_id,'full');   
