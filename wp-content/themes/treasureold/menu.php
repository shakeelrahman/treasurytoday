...
function create_bootstrap_menu($theme_location)
{
  ...
  if (($theme_location) && ($locations = get_nav_menu_locations()) && isset($locations[$theme_location])) {
    ...
    $menu_list .=  '<a href="' . get_home_url() . '" class="border-end logo-link py-lg-0 py-2 pe-lg-3">';
    $menu_list .=   $logoset . '</a>
    ...
    foreach ($menu_items as $menu_item) {
      ...
      if ($menu_item->menu_item_parent == 0) {
        ...
        if ($bool == true && count($menu_array) > 0) {
          ...
          $menu_list .= '<a id="navbarDropdown" class="nav-link dropdown-toggle py-lg-0 d-flex align-items-center justify-content-lg-center" data-bs-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">' . $menu_item->title . '<i class="ps-2 fa-solid fa-chevron-down"></i></a>' . "\n";
